...
class AddressController extends Controller{
	...
	public function address_list(){
		...
		$res = DB::table('lat_hotel')->skip($offset)->take($num)->where('city_id','like',"$city_id")->get();
	    ...
	    foreach ($arr as $k => $v) {
	    	 $arr1[] = DB::table('lat_region')->whereIn('region_id',$v)->lists('region_name');
	    }
	   ...
	   // print_r($res);die;
		...
		$res = DB::table('lat_hotel')->skip($offset)->take($num)->get();
	...
	// 	$res = DB::table('lat_hotel')->where('city_id','like','%$city_id%')->get();
 ...
 //      	}
     ...
     // 	$res=DB::delete('delete from lat_hotel where hotel_id=:hotel_id',['hotel_id'=>$hotel_id]);
		   //  if($res)
	    // { 
	...
	public function address_josn(){
		...
		$province=DB::table('lat_region')->where('region_type','=',1)->get();
        $city=DB::table('lat_region')->where('region_type','=',2)->get();
        $county=DB::table('lat_region')->where('region_type','=',3)->get();
