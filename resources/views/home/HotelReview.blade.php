...
<div class="hotel-comment-list">
	 @foreach($arr as $v)
      <div class="hotel-user-comment">
				<span class="hotel-user">
					<img width="32" height="32" src="{{$v['user_file']}}">
					<?php if($v['user_num']<1000){?>
					用户：{{$v['user_name']}}
					<?php }elseif($v['user_num']>=1000 && $v['user_num']<2000){ ?>
					会员：{{$v['user_name']}}
					<?php }elseif($v['user_num']>=2000 && $v['user_num']<4000){ ?>
					银卡会员：{{$v['user_name']}}
					<?php }elseif($v['user_num']>=4000 && $v['user_num']<8000){ ?>
					金卡会员：{{$v['user_name']}}
					<?php }else{ ?>
					钻石会员：{{$v['user_name']}}
					<?php } ?>
...
<script>
	function page(page){
		$.get("HotelReview",{page:page,hotel_id:{{$hotel_id}}},function(e){
			$("#div1").html(e);
		});
	}
</script>
