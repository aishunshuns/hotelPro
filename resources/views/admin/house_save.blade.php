@include('admin/header')
     <div class="page-content"> 
      <div class="page-header"> 
       <h1>酒店户型修改<small> <i class="icon-double-angle-right"></i></small> </h1> 
      </div>
      <!-- /.page-header --> 
      <div class="row"> 
       <div class="col-xs-12"> 
        ...
        <div class="row"> 
         <div class="col-xs-12"> 
          ...
          <form action="{{URL('houseUpdate')}}" method="post" enctype="multipart/form-data">
          <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
          <input type="hidden" name="h_id" value="{{$arr['arr']['h_id']}}">
           <table id="sample-table-1" class="table table-striped table-bordered table-hover"> 
              <tr>
                ...
                <td>
                  ...
                  <option value="">--请选择--</option>
                    @foreach($arr['hotel'] as $v)
                      <option value="{{$v['hotel_id']}}" <?php if($v['hotel_id']==$arr['arr']['hotel_id']){ echo "selected";} ?> >{{$v['hotel_name']}}</option>
                    @endforeach
                  </select>
                </td>
              </tr>
              <tr>
                ...
                <td>
                  <select name="house_id" id="house_id">
                    <option value="">--请选择--</option>
                    @foreach($arr['house'] as $v)
                    <option value="{{$v['house_id']}}"  <?php if($v['house_id']==$arr['arr']['house_id']){ echo "selected";} ?> >{{$v['house_name']}}</option>
                    @endforeach
                  </select>
                </td>
              </tr>
              <tr>
                <th>户型行情价：</th>
                <td>
                  <input type="text" name="house_price" id="house_price" value="{{$arr['arr']['house_price']}}">元
                </td>
              </tr>
              <tr>
                <th>普通会员价：</th>
                <td>
                  <input type="text" name="member_price" id="member_price" value="{{$arr['arr']['member_price']}}">元
                </td>
              </tr>
              <tr>
                <th>银卡会员价：</th>
                <td>
                  <input type="text" name="silver_price" id="silver_price" value="{{$arr['arr']['silver_price']}}">元
                </td>
              </tr>
              <tr>
                <th>金卡会员价：</th>
                <td>
                    <input type="text" name="gold_price" id="gold_price" value="{{$arr['arr']['gold_price']}}">元
                </td>
              </tr>
              <tr>
                <th>钻石会员价：</th>
                <td>
                    <input type="text" name="diamond_price" id="diamond_price" value="{{$arr['arr']['diamond_price']}}">元
                </td>
              </tr>
              <tr>
                <th>户型房间数量：</th>
                <td>
                  <input type="text" name="house_num" id="house_num" value="{{$arr['arr']['house_num']}}">
                </td>
              </tr>
              <tr>
                <th>户型图片：</th>
